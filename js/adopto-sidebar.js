(function ($, sidebar, rootUrl) {
    ...
    sidebar.setFormData = function (cdata) {
        ...
        $('.adopto-sidebar .adopto-group .adopto-group-title').unbind();
        $('.adopto-sidebar a').unbind();
        var titles = $('.adopto-sidebar .adopto-group .adopto-group-title');
        for (var i = 0; i < titles.length; i++) {
            $(titles[i]).parent().removeClass('collapsed');
        }
        ...
        for (var i = 0; i < cdata.mainData.education.length; i++) {
            ...
            $('#eduItem' + i).click(sidebar.openDetailsPane.bind(e, 'Education'));
        }
        ...
        for (var i = 0; i < cdata.mainData.experience.length; i++) {
            ...
            $('#expItem' + i).click(sidebar.openDetailsPane.bind(e, 'Experience'));
        }
        ...
        for (var i = 0; i < cdata.mainData.projects.length; i++) {
            ...
            $('#projItem' + i).click(sidebar.openDetailsPane.bind(p, 'Project'));
        }
        ...
        var anchors = $('#tabContent a.atPlace');
    ...
    sidebar.openDetailsPane = function (title) {
        //console.log(title);
        $('.backPane span').html(title);
        sidebar.scrollTop = $('#tabContent').scrollTop();
        $('.tab-header[data-tab!="tabForm"]').hide();
    ...
    sidebar.hideEmptyGroups = function () {
        var anchors = $('#tabContent a.side-desc');
        console.log(anchors);
        for (var i = 0; i < anchors.length; i++) {
            if ($(anchors[i]).attr('href') == '#') {
                $(anchors[i]).addClass('empty');
                ...
                .done(function (data) {
                        ...
                        .done(function (data) {
                            ...
                            $('#tabContent').bind('mousewheel', function (e) {
                                if ($('.tab-header[data-tab="tabForm"').hasClass('active')) {
                                    if ($('.tabForm.tab').css('right') != '0px' || (e.originalEvent.deltaY > 0 &&
                                        $(this).scrollTop() >= $('#adopto-form').outerHeight() - $(this).outerHeight() + 40) ||
                                    ...
                                    if ((e.originalEvent.deltaY > 0 &&
                                        $(this).scrollTop() === $('.tabActivities.tab .activities-group').outerHeight() - $(this).outerHeight() + 10) ||
                                        (e.originalEvent.deltaY < 0 && $(this).scrollTop() === 0)) {
                                        e.preventDefault();
                                    }
                                }
                                    ...
                                    //console.log($('.tabCommunication.tab .communications').outerHeight() - $(this).outerHeight() + 10);
                                    if ((e.originalEvent.deltaY > 0 &&
                                        $(this).scrollTop() === $('.tabCommunication.tab .communications').outerHeight() - $(this).outerHeight() + 20) ||
                                        (e.originalEvent.deltaY < 0 && $(this).scrollTop() === 0)) {
                                        e.preventDefault();
                                    }
                                }
                            ...
                            $('.mainContent .done-icon').click(function () {
                                ...
                                candidateDataModel.mainData.fullName = $('.adopto-sidebar .mainContent .curr-info-edit input.name').val();
                                candidateDataModel.mainData.title = $('.adopto-sidebar .mainContent .curr-info-edit input.jobTitle').val();
                                candidateDataModel.mainData.location = $('.adopto-sidebar .mainContent .curr-info-edit input.location').val();

                                $('.adopto-sidebar .mainContent p.name').html(candidateDataModel.mainData.fullName);
                                $('.adopto-sidebar .mainContent h5.jobTitle').html(candidateDataModel.mainData.title);
                                $('.adopto-sidebar .mainContent p.location span').html(candidateDataModel.mainData.location);
                            });

                            $('.adopto-sidebar .curr-info-edit').on('keypress', 'input', function (e) {
                                if (e.which == 13) {
                                    $('.mainContent .done-icon').click();
                                    return false;
                                }
                            });
                            ...
                            $('.topHeader .backPane').click(function () {
                                ...
                                $('#tabContent').scrollTop(sidebar.scrollTop);
                                ...
                                $('.topHeader .backPane').animate({ opacity: 0 }, 100, function () {
                                    ...
                                    $('.tab-header[data-tab!="tabForm"]').show();
                                });
                            });
                    ...
                    $.get(chrome.extension.getURL('markup/communication.html'))
                        .done(function (data) {
                            ...
                            $('#emailTab button').attr('data-original-title', '');
